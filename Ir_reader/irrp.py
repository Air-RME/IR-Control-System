...
#    return True
def compare(p1, p2):
    hex_p1 = hex.ir_codes_conversion(p1)
    hex_p2 = hex.ir_codes_conversion(p2)
    for i in range(len(hex_p1)):
        if hex_p1[i] != hex_p2[i]:
            return False
    return True
...
if args.record:  # Record.
    ...
    while not done:
        for arg in args.id:
            ...
            time.sleep(1)
            records[arg] = code[:]
        for arg in (reversed(args.id)):
            tries = 0
            while True:
                print("Check Press key for '{}'".format(arg))
                code = []
                fetching_code = True
                while fetching_code:
                    time.sleep(0.1)
                the_same = compare(records[arg], code[:])

                if the_same:
                    print("Check Okay '{}'".format(arg))
                    time.sleep(1)
                    break
                elif tries <= 3:
                    print("Nomatch")
                    ...
                    print("ERROR")
                    time.sleep(4)
                    print("Retry Press key for '{}'".format(arg))
                    code = []
                    fetching_code = True
                    while fetching_code:
                        time.sleep(0.1)
